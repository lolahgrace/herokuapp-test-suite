...
test.describe('Number inputs functionality', () => {
    ...
    test('Enter a valid number and verify it appears in the field', async ({ page }) => {
    ...
    test('Enter a negative number and verify it is accepted', async ({ page }) => {
        ...
        await test.step('Verify it is accepted', async () => {
            await expect(inputsPage.numberInputs).toHaveValue(testData.negativeNumber);
        });
    });
    ...
    test('Handle increment via arrow keys', async ({ page }) => {
        ...
        await test.step('Verify number increase', async () => {
            await expect(inputsPage.numberInputs).toHaveValue(String(Number(testData.validNumber) + 1));
        });
    });
    ...
    test('Type special characters and verify field rejects them', async ({ page }) => {
